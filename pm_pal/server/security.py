...
Prefer PM_PAL_* env vars; MARRDP_* remains a compatibility fallback. :-)
"""
...
# Exact webhook event paths that may skip API-key auth after signature verification
# inside the provider router. Do not exempt whole /api/{provider}/ prefixes. :-)
WEBHOOK_EVENT_PATHS = frozenset(
    {
        "/api/feishu/events",
        "/api/github/events",
        "/api/notion/events",
...
class ApiSecuritySettings:
    # Desktop-local default: auth off. Shared deployments set PM_PAL_API_AUTH_DISABLED=false. :-)
...
def security_settings() -> ApiSecuritySettings:
    return ApiSecuritySettings(
        auth_disabled=_env_flag(
            "PM_PAL_API_AUTH_DISABLED",
            "MARRDP_API_AUTH_DISABLED",
            default=True,
        ),
        api_key=_env_first("PM_PAL_API_KEY", "MARRDP_API_KEY"),
        bearer_token=_env_first("PM_PAL_API_BEARER_TOKEN", "MARRDP_API_BEARER_TOKEN"),
        rate_limit_disabled=_env_flag(
            "PM_PAL_API_RATE_LIMIT_DISABLED",
            "MARRDP_API_RATE_LIMIT_DISABLED",
            default=True,
        ),
        rate_limit_max_requests=_env_int(
            "PM_PAL_API_RATE_LIMIT_MAX_REQUESTS",
            "MARRDP_API_RATE_LIMIT_MAX_REQUESTS",
            default=5,
            minimum=1,
        ),
        rate_limit_window_sec=_env_int(
            "PM_PAL_API_RATE_LIMIT_WINDOW_SEC",
            "MARRDP_API_RATE_LIMIT_WINDOW_SEC",
            default=60,
            minimum=1,
        ),
    )
...
def validate_security_at_startup(settings: ApiSecuritySettings | None = None) -> None:
    """Fail fast when auth is enabled but no API credentials are configured."""
    resolved = settings or security_settings()
    if resolved.auth_disabled:
        return
    if not resolved.api_key and not resolved.bearer_token:
        raise RuntimeError(
            "API authentication is enabled but PM_PAL_API_KEY (or PM_PAL_API_BEARER_TOKEN) "
            "is missing. Set a credential or explicitly set PM_PAL_API_AUTH_DISABLED=true."
        )
...
def is_webhook_event_path(path: str) -> bool:
    normalized = str(path or "").rstrip("/") or "/"
    return normalized in WEBHOOK_EVENT_PATHS
    ...
    if not settings.api_key and not settings.bearer_token:
        return controlled_error_response(
            ...
            message=(
                ...
                "Set PM_PAL_API_KEY or PM_PAL_API_BEARER_TOKEN, or opt out with "
                "PM_PAL_API_AUTH_DISABLED=true."
            ),
        )
