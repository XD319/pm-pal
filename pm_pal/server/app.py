...
DATA_ROOT = Path(os.getenv("PM_PAL_DATA_DIR", "data"))
OUTPUTS_ROOT = DATA_ROOT / "outputs"
WORKSPACE_DB_PATH = DATA_ROOT / "workspace.sqlite3"
PROJECT_SPACE_DB_PATH = DATA_ROOT / "project_space.sqlite3"
...
_connector_sync_worker: ConnectorSyncWorker | None = None
...
async def _app_lifespan(app: FastAPI):
    ...
    worker = _connector_sync_worker
    if worker is not None:
        worker.start()
    app.state.connector_sync_worker = worker
...
app = FastAPI(title="PM Pal Project Space API", version="5.0", lifespan=_app_lifespan)
...
def _data_root_writable() -> bool:
    DATA_ROOT.mkdir(parents=True, exist_ok=True)
    probe_path = DATA_ROOT / ".ready-probe"
    probe_path.write_text("ok", encoding="utf-8")
    probe_path.unlink(missing_ok=True)
    return True


def _project_space_db_available() -> bool:
    import sqlite3

    PROJECT_SPACE_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(PROJECT_SPACE_DB_PATH) as connection:
...
async def _api_security_middleware(request: Request, call_next):
    if not request.url.path.startswith("/api/"):
        return await call_next(request)
    # Only signed webhook event endpoints skip API-key auth. :-)
    if is_webhook_event_path(request.url.path):
        return await call_next(request)
...
def healthcheck() -> dict[str, Any]:
    ...
    worker_snapshot = worker.snapshot() if worker is not None else {"alive": False}
    return {
        "ok": True,
        "status": "healthy",
        "service": "pm-pal",
        "deployment": {
            ...
            "note": "SSE progress is in-process only; run a single replica.",
        },
        "project_space": {
            "db_path": str(PROJECT_SPACE_DB_PATH),
            "data_dir": str(DATA_ROOT),
        },
        "connector_worker": worker_snapshot,
    }
...
def readiness_check() -> JSONResponse:
    ...
    db_available = False
    worker = getattr(app.state, "connector_sync_worker", None)
    worker_alive = bool(worker is not None and worker.alive and worker.running)
    errors: list[str] = []
    ...
    try:
        data_writable = _data_root_writable()
    except Exception as exc:
        errors.append(f"data_root_unwritable: {exc}")
    try:
        db_available = _project_space_db_available()
    except Exception as exc:
        errors.append(f"project_space_db_unavailable: {exc}")
    if not worker_alive:
        errors.append("connector_sync_worker_not_alive")

    ready = (
        startup_completed
        and outputs_writable
        and data_writable
        and db_available
        and worker_alive
    )
    payload = {
        ...
        "checks": {
            ...
            "project_space_db_available": db_available,
            "connector_sync_worker_alive": worker_alive,
...
(
    _project_space_router,
    _feishu_config_store,
    _connector_sync_store,
    _github_config_store,
    _notion_config_store,
    _project_store,
    _project_domain_repository,
...
_connector_sync_worker = ConnectorSyncWorker(
    _connector_sync_store,
    _project_store,
    new_id=new_id,
    now=now,
