...
class PresetCreate(BaseModel):
    ...
    def resolved_model(self) -> str:
        for candidate in (self.model, self.smart_model, self.fast_model, self.strategic_model):
...
""")
            ...
            preset_cols = {row[1] for row in c.execute("PRAGMA table_info(model_presets)").fetchall()}
            if "model" not in preset_cols:
                c.execute("ALTER TABLE model_presets ADD COLUMN model TEXT NOT NULL DEFAULT ''")
                c.execute(
...
class SecretBox:
    ...
    def encrypt(self, secret: str) -> str:
        ...
        if not self.box: raise HTTPException(400, detail="Set PM_PAL_SECRETS_MASTER_KEY before saving a provider API key.")
        return self.box.encrypt(secret.encode()).decode()
    def decrypt(self, token: str) -> str:
        if not token: return ""
        if not self.box: raise HTTPException(503, detail="Provider secrets are locked: PM_PAL_SECRETS_MASTER_KEY is not configured.")
...
def create_project_space_router(
    *,
    ...
    def get_project(project_id):
        ...
        item = rows[0]
        item["delivery_target"] = json.loads(item.pop("delivery_target_json", "{}") or "{}")
        item["source_bindings"] = json.loads(item.pop("source_bindings_json", "[]") or "[]")
        item["sources"] = [
            public_source(row)
    ...
    async def provider_catalog():
        return {"providers": [_provider_catalog_entry(p) for p in sorted(SUPPORTED_PROVIDERS)]}
    ...
    async def create_connection(p: ConnectionCreate):
        if p.provider not in SUPPORTED_PROVIDERS: raise HTTPException(400, detail="Unsupported provider")
    ...
    def save_preset(preset_id, p: PresetCreate):
        ...
        return _public_preset(store.rows("SELECT * FROM model_presets WHERE id=?", (preset_id,))[0])
    ...
    async def create_project(p: ProjectCreate):
        ...
        store.execute("INSERT INTO projects (id,name,description,model_preset_id,created_at,updated_at,product_id,owner_actor,delivery_target_json,source_bindings_json) VALUES (?,?,?,?,?,?,?,?,?,?)", (project_id, p.name.strip(), p.description.strip(), p.model_preset_id, stamp, stamp, p.product_id.strip(), p.owner_actor.strip(), json.dumps(p.delivery_target), json.dumps(p.source_bindings)))
    ...
    async def update_project(project_id: str, p: ProjectUpdate):
        ...
        store.execute("UPDATE projects SET name=?,description=?,model_preset_id=?,product_id=?,owner_actor=?,delivery_target_json=?,source_bindings_json=?,updated_at=? WHERE id=?", (p.name.strip() if p.name is not None else old["name"], p.description.strip() if p.description is not None else old["description"], p.model_preset_id if p.model_preset_id is not None else old["model_preset_id"], p.product_id.strip() if p.product_id is not None else old.get("product_id", ""), p.owner_actor.strip() if p.owner_actor is not None else old.get("owner_actor", ""), json.dumps(p.delivery_target if p.delivery_target is not None else old["delivery_target"]), json.dumps(p.source_bindings if p.source_bindings is not None else old["source_bindings"]), now(), project_id))
        return get_project(project_id)

    @router.get("/projects/{project_id}/context")
    async def project_context(project_id: str):
        item = get_project(project_id)
        return {"project_id": project_id, "product_id": item.get("product_id", ""), "owner_actor": item.get("owner_actor", ""), "model_preset_id": item.get("model_preset_id"), "delivery_target": item["delivery_target"], "source_bindings": item["source_bindings"], "connectors": list_connector_summaries(sync_store, project_id=project_id), "stats": {"sources": len(item["sources"]), "runs": len(item["runs"])}}
    ...
    async def add_source_from_url(project_id: str, p: SourceFromUrl):
        ...
        try:
            ...
            if payload is None:
                raise HTTPException(status_code=400, detail=str(exc) or "Failed to fetch source.") from exc
            status_by_code = {
            ...
            raise HTTPException(
                ...
                detail={"code": str(payload.code), "message": message, "source": payload.source},
            ) from exc
        source_type = (
    ...
    return router, feishu_config_store, sync_store, github_config_store, notion_config_store, store, domain_repository
